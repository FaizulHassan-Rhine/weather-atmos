...
import { formatMagnitude } from "@/lib/earthquake";
    ...
    >
      ...
      {!events?.length ? (
        <p className="px-5 py-8 text-sm text-muted-soft">
          {t("No disaster events match these filters.")}
        </p>
      ) : (
        <ul className="max-h-[28rem] divide-y divide-sky/15 overflow-y-auto dark:divide-white/8">
          {events.map((event) => {
            ...
            const tone =
              SEVERITY_TONE_CLASSES[event.severity] ||
              SEVERITY_TONE_CLASSES.muted;
            return (
              <li key={event.id}>
                <button
                  type="button"
                ...
                >
                  <span
                    className="mt-1 flex h-11 w-11 shrink-0 flex-col items-center justify-center rounded-xl text-xs font-semibold text-white"
                    style={{
                      backgroundColor:
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-soft">
                      ...
                      <span>{t(sourceLabel(event.source))}</span>
